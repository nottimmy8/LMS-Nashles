...
import { getPopularCategories } from "@/services/analytics-service";
import {
  BarChart,
  Bar,
  XAxis,
...
export function PopularCoursesChart() {
  const [data, setData] = useState<{ course: string; students: number }[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const mappedData = response.categories.map((item: any) => ({
          course: item.category || "Uncategorized",
  ...
  return (
    <Card className="glass-panel border-white/5 shadow-2xl rounded-[2.5rem] overflow-hidden p-6 transition-all hover:border-white/10 group relative">
      ...
      <CardContent className="relative z-10 h-75">
        {loading ? (
          ...
          <div className="flex items-center justify-center h-full text-white/20 font-bold uppercase tracking-widest text-xs">
            No data available
          </div>
        ) : (
